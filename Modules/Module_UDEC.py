...
from UDEC_Module.UDEC import DataSet
import argparse

class HODS_Module (DemModuleBaseClass):
    def __init__(self):
        program = 'python UDEC.py'
        parameters = {}
        DemModuleBaseClass.__init__(self, program, parameters)
    ...
    def run(self):
        ...
        for i in range(len(dataClass[0].keys())):
            H = Homogenize(self.revCentre, self.revRadius, dataClass=dataClass)
